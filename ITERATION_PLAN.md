...
> **状态：✅ 已完成（2026-09-21，v1.0.0，tag `v1.0.0`）——一期完成**。DoD：ARCHITECTURE §10 一期完成要求 8 条逐条核对通过并留证（[docs/ACCEPTANCE.md](docs/ACCEPTANCE.md)：三市场承诺类型文件可用含 20-F 发行人、重跑不重复、修订不覆盖、失败隔离、HTTP 全链路、错误可区分、文档一致、性能记录制）。三市场集成冒烟矩阵：6 只 ×4 = 24/24（样本日期/URL/类型留档）；性能基线：首跑 121s、缓存 18s、136MiB、2 次重试；README + curl/Python 调用示例 + 已知限制清单交付；文档-实现一致性核对完成（含 6 项实现期偏差记录）。
...
| QTR-HK 默认启用及混合选择修复 | [HK_QUARTERLY_DEFAULT_TASK.md](HK_QUARTERLY_DEFAULT_TASK.md) | **✅ 已完成（2026-09-23，v1.0.3，RF-HK-QTR-DEFAULT-001）**：两阶段混合选择（Store 回填 + 有界判期预取）落地，冷/热库一致；文档/契约/版本已同步；生产部署状态见 `DEPLOY.md` |
