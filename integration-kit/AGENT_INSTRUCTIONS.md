...
14. 有效默认值幂等：省略 forms 与显式默认 forms 视为同一请求（v1.0.3 的 HK
    默认含 `QTR-HK`；显式 `{"HK":["ANNUAL","INTERIM"]}` 是不同请求）。
15. HK 季度语义：mock 发行人末位偶数代码有季度业绩（0700.HK），奇数没有
    （0005.HK）。无季度发行人的默认请求正常降级；显式 `{"HK":["QTR-HK"]}`
    对其是正常 `no_reports`，不是失败。
