...
> 后续验收（2026-09-21）：v1.0.1 / `76fee13` 已完成 T1–T7 独立复审（178 项定向测试通过）并部署目标服务器；三市场各 1 份真实 HTTP 抓取、同键重放、全缓存复用、文件哈希及重启读取均通过。HK 因报告期未知保留 partial，3 份原文均可用。实际版本、环境、任务 ID 与运维方式见 [DEPLOY.md](../DEPLOY.md)。以下 v1.0.0 数据保留为首次验收历史。
