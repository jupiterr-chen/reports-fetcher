...
- 判期证据回填（**RF-HK-PERIOD-EVIDENCE-001，2026-09-26**）：部分发行人（实测小米 1810）历年定期报告 PDF 正文缺 ToUnicode 映射，文本判期恒不可用（cid 乱码，英文版不存在）。業績检索中的 **[中期業績]/[末期業績] 公告标题自带明确期末日**，作为文档判期失败后的兜底：`period_source=announcement_title`，证据来源记录于 `source_metadata.period_evidence`；优先级 document > announcement_title；单年标签 + 该 (doc_type, 年份) 证据期末唯一才回填，跨年标签与歧义证据保持 unknown；業績公告本身不作为归档候选（公告已发、完整报告未发的窗口期仍取不到中报，属已知边界）；業績检索因证据需要在 HK forms 含任一类型时始终执行。
